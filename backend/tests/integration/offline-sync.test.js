...
describe('离线模式同步功能集成测试', () => {
    ...
    test('17.8 用户手动清理缓存后保留最近7天的核心数据', async () => {
        ...
        const itemsToKeep = cacheItems.filter(item => item.type === 'core' && item.created_at >= sevenDaysAgo);
        expect(itemsToKeep.length).toBe(2);
        expect(itemsToKeep.map(item => item.id)).toContain('core_1');
        expect(itemsToKeep.map(item => item.id)).toContain('core_2');
        // 验证其他数据被删除
        const itemsToDelete = cacheItems.filter(item => !(item.type === 'core' && item.created_at >= sevenDaysAgo));
...
async function cleanupTestData() {
    try {
        ...
        // 删除班级
        await connection.execute('DELETE FROM classes WHERE name = ?', ['离线模式测试班级']);
        // 删除用户
        if (studentId) {
