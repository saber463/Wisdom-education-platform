 ...
 * 验证服务顺序是否正确
 */
export function validateStartupOrder(services) {
 ...
 * 检查服务是否正在运行
 */
export async function isServiceRunning(service) {
 ...
 * 通过检查端口占用情况来推断启动顺序
 */
export async function verifyStartupSequence() {
 ...
 * 检查所有服务是否已启动
 */
export async function areAllServicesRunning() {
    const runningServices = await getRunningServices();
 ...
 * 获取下一个应该启动的服务
 */
export async function getNextServiceToStart() {
    const runningServices = await getRunningServices();
    const expectedOrder = getStartupOrder();
...
//# sourceMappingURL=startup-order.js.map