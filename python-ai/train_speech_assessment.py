...
# -*- coding: utf-8 -*-
"""
Wav2Vec2口语评测模型训练脚本（CPU极致优化版）
功能：基于真实音频数据训练Wav2Vec2模型，用于口语评测
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('train_speech_assessment.log'),
...
OUTPUT_DIR = "./models/speech_assessment"
PROCESSED_DATA_DIR = "./data/processed/speech_assessment_dataset"

# CPU优化训练参数（音频模型更消耗资源）
BATCH_SIZE = 2  # 音频模型批次更小
...
def preprocess_function(examples, processor):
    """处理音频特征数据"""
    # 音频特征已在data_preprocess.py中提取为MFCC
    # 这里直接使用预处理的特征
    audio_features = examples["audio_feature"]
    
    # 转换为numpy数组并填充/截断到固定长度
    processed_features = []
    for feature in audio_features:
        feature_array = np.array(feature)
        
        # 填充或截断
        if len(feature_array) < MAX_AUDIO_LENGTH:
            # 填充
            padded = np.zeros((MAX_AUDIO_LENGTH, feature_array.shape[1]))
            padded[:len(feature_array)] = feature_array
            processed_features.append(padded)
        else:
            # 截断
            processed_features.append(feature_array[:MAX_AUDIO_LENGTH])
...
def main():
    logger.info("="*60)
    logger.info("开始训练Wav2Vec2口语评测模型（CPU优化版）")
    logger.info("="*60)
    ...
    # 2. 加载processor和模型
    logger.info("加载Wav2Vec2模型和processor...")
    
    try:
        processor = Wav2Vec2Processor.from_pretrained(MODEL_NAME)
    except Exception as e:
        logger.warning(f"加载processor失败: {e}")
        logger.info("使用默认配置...")
        processor = None
    ...
    # 由于音频特征已在data_preprocess.py中提取，这里直接使用
    # 重命名列以匹配模型输入
    def prepare_dataset(examples):
        # 将audio_feature转换为input_values
        features = []
        for feature in examples["audio_feature"]:
            feature_array = np.array(feature, dtype=np.float32)
            # 填充或截断到固定长度
            if len(feature_array) < 1000:
                padded = np.zeros((1000, 20), dtype=np.float32)
                padded[:len(feature_array)] = feature_array
                features.append(padded.flatten())
            else:
                features.append(feature_array[:1000].flatten())
    ...
    training_args = TrainingArguments(
        ...
        weight_decay=0.01,
        logging_dir='./logs/speech_assessment',
        logging_steps=50,
        eval_strategy="epoch",
        save_strategy="epoch",
    ...
    try:
        ...
        logger.info("提示：音频模型训练需要较大内存，建议减小batch_size或使用更小的数据集")
        sys.exit(1)
    ...
    eval_result = trainer.evaluate(processed_dataset['test'])
    ...
    logger.info("="*60)
    logger.info("✓ Wav2Vec2口语评测模型训练完成！")
    logger.info("="*60)
