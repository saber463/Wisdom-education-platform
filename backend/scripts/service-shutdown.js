 ...
 * 停止占用指定端口的所有进程
 */
export function stopProcessesByPort(port) {
 ...
 * 检查所有服务是否已停止
 */
export async function areAllServicesStopped() {
 ...
 * 获取正在运行的服务端口列表
 */
export async function getRunningPorts() {
    const runningPorts = [];
    for (const service of SERVICES) {
        const inUse = await isPortInUse(service.port);
        if (inUse) {
...
//# sourceMappingURL=service-shutdown.js.map