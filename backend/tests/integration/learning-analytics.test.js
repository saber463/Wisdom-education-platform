...
async function setupTestData() {
    try {
       ...
       VALUES (?, ?, ?, ?, ?)`, ['test_teacher_analytics', 'hash123', '学情分析测试教师', 'teacher', 'active']);
       ...
       VALUES (?, ?, CURDATE())`, [classId, studentId]);
        // 创建知识点
        const [kpResult] = await connection.execute(`INSERT INTO knowledge_points (name, subject, grade)
       VALUES (?, ?, ?)`, ['一次函数', '数学', '高一']);
        knowledgePointId = kpResult.insertId;
        // 创建作业
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [
        ...
        // 创建作业提交
        const [submissionResult] = await connection.execute(`INSERT INTO submissions (assignment_id, student_id, file_url, status, total_score, grading_time)
       VALUES (?, ?, ?, ?, ?, NOW())`, [assignmentId, studentId, '/uploads/test.pdf', 'graded', 78]);
...
async function cleanupTestData() {
    try {
        // 删除提交记录
        if (submissionId) {
            await connection.execute('DELETE FROM submissions WHERE id = ?', [submissionId]);
        }
        // 删除作业
        if (assignmentId) {
        ...
        // 删除知识点
        if (knowledgePointId) {
            await connection.execute('DELETE FROM knowledge_points WHERE id = ?', [knowledgePointId]);
        }
        // 删除用户
        if (teacherId) {
...
//# sourceMappingURL=learning-analytics.test.js.map