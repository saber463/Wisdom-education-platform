...
import { DEFAULT_BACKUP_CONFIG, isMySQLAvailable, isMySQLDumpAvailable, databaseExists, ensureBackupDirectory, generateBackupFileName, listBackupFiles, getLatestBackup, getBackupStats } from '../database-backup.js';
        ...
        fc.assert(fc.property(fc.constant(null), () => {
            ...
            if (files.length > 1) {
                ...
                for (let i = 0; i < files.length - 1; i++) {
                    expect(files[i].createdAt.getTime()).toBeGreaterThanOrEqual(files[i + 1].createdAt.getTime());
                }
            }
            return true;
        }), { numRuns: 10 });
...
//# sourceMappingURL=database-backup.property.test.js.map