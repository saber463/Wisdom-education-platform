...
import { findProcessByPort, killProcess, stopProcessesByPort, areAllServicesStopped, getRunningPorts, cleanupTempFiles } from '../service-shutdown.js';
...
describe('Property 52: 服务安全关闭', () => {
    ...
    it('属性2: 停止进程函数应该返回布尔值', () => {
        fc.assert(fc.property(fc.integer({ min: 1, max: 99999 }), (pid) => {
    ...
    it('属性9: 查找不存在的端口应该返回空数组', () => {
        fc.assert(fc.property(fc.integer({ min: 60000, max: 65535 }), // 使用不太可能被占用的高端口
        (port) => {
            const pids = findProcessByPort(port);
            // 验证：返回值应该是数组（可能为空）
            expect(Array.isArray(pids)).toBe(true);
    ...
    it('属性10: 停止不存在的进程应该返回false', () => {
        fc.assert(fc.property(fc.integer({ min: 90000, max: 99999 }), // 使用不太可能存在的PID
        (pid) => {
            const result = killProcess(pid);
...
//# sourceMappingURL=service-shutdown.property.test.js.map