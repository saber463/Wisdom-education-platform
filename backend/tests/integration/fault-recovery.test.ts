  ...
  describe('数据库连接失败自动重连', () => {
    ...
    test('10.7 测试数据库连接池健康检查', async () => {
      ...
      try {
        ...
        expect((result as any[])[0].health_check).toBe(1);
  ...
  describe('服务崩溃自动重启', () => {
    ...
    test('10.6 模拟服务健康检查失败', async () => {
      ...
      try {
        ...
        checkFailed = true;
        expect((error as Error).message).toBe('Service unhealthy');
      }
...
// 辅助函数：查找可用端口
async function findAvailablePort(defaultPort: number, alternatives: number[]): Promise<number> {
