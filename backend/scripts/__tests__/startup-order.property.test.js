...
import { getStartupOrder, validateStartupOrder, validateServiceDefinitions, getServiceInfo, isPortInUse, verifyStartupSequence } from '../startup-order.js';
describe('Property 50: 服务启动顺序正确性', () => {
    ...
    it('属性4: 服务信息查询应该返回正确的服务对象', () => {
        fc.assert(fc.property(fc.constantFrom('MySQL', 'Rust', 'Python', 'Node', 'Frontend'), (serviceName) => {
    ...
    it('属性5: 不存在的服务名应该返回undefined', () => {
        fc.assert(fc.property(fc.string().filter(s => !['MySQL', 'Rust', 'Python', 'Node', 'Frontend'].includes(s)), (invalidServiceName) => {
    ...
    it('属性6: 端口检查函数应该返回布尔值', async () => {
        await fc.assert(fc.asyncProperty(fc.integer({ min: 1024, max: 65535 }), async (port) => {
    ...
    it('属性10: 服务列表应该是不可变的（每次调用返回相同的顺序）', () => {
        fc.assert(fc.property(fc.integer({ min: 2, max: 10 }), (iterations) => {
...
//# sourceMappingURL=startup-order.property.test.js.map