...
then
    ...
    echo "可以通过Homebrew安装：brew install node@18"
    read -p "按任意键退出..."
    exit 1
fi
...
then
    ...
    echo "可以通过Homebrew安装：brew tap mongodb/brew && brew install mongodb-community"
    echo "启动MongoDB：brew services start mongodb-community"
    read -p "按Enter键继续安装依赖..."
else
    echo "MongoDB已安装"
    echo ""
fi
...
if [ ! -f "server/.env" ]; then
    ...
    cp "server/.env" "server/.env.example" 2>/dev/null
    if [ $? -ne 0 ]; then
