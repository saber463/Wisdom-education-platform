 ...
 * 检查数据库是否存在
 */
export function databaseExists(config = DEFAULT_DB_CONFIG) {
    try {
 ...
 * 删除数据库
 */
export function dropDatabase(config = DEFAULT_DB_CONFIG) {
    try {
 ...
 * 创建数据库
 */
export function createDatabase(config = DEFAULT_DB_CONFIG) {
    try {
 ...
 * 导入SQL文件
 */
export function importSQLFile(filePath, config = DEFAULT_DB_CONFIG) {
    try {
 ...
 * 获取表数量
 */
export function getTableCount(config = DEFAULT_DB_CONFIG) {
    try {
 ...
 * 获取表中的记录数
 */
export function getRecordCount(tableName, config = DEFAULT_DB_CONFIG) {
    try {
 ...
 * 获取演示数据统计
 */
export function getDemoDataStats(config = DEFAULT_DB_CONFIG) {
 ...
 * 重置演示数据
 */
export function resetDemoData(config = DEFAULT_DB_CONFIG) {
    const steps = {
        dropDatabase: false,
 ...
 * 验证演示数据完整性
 */
export function validateDemoData(config = DEFAULT_DB_CONFIG) {
    const errors = [];
    const stats = getDemoDataStats(config);
    // 检查数据库是否存在
    if (!databaseExists(config)) {
...
//# sourceMappingURL=demo-data-manager.js.map