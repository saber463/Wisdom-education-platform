 ...
 * 检查mysqldump是否可用
 */
export function isMySQLDumpAvailable(): boolean {
  try {
 ...
 * 检查数据库是否存在
 */
export function databaseExists(config: BackupConfig = DEFAULT_BACKUP_CONFIG): boolean {
  try {
 ...
 * 确保备份目录存在
 */
export function ensureBackupDirectory(config: BackupConfig = DEFAULT_BACKUP_CONFIG): boolean {
  try {
 ...
 * 执行数据库备份
 */
export function backupDatabase(config: BackupConfig = DEFAULT_BACKUP_CONFIG): BackupResult {
 ...
 * 获取所有备份文件
 */
export function listBackupFiles(config: BackupConfig = DEFAULT_BACKUP_CONFIG): BackupFileInfo[] {
 ...
 * 获取最新的备份文件
 */
export function getLatestBackup(config: BackupConfig = DEFAULT_BACKUP_CONFIG): BackupFileInfo | null {
  const backupFiles = listBackupFiles(config);
 ...
 * 获取备份统计信息
 */
export function getBackupStats(config: BackupConfig = DEFAULT_BACKUP_CONFIG): {
  totalBackups: number;
  totalSize: number;
  oldestBackup?: Date;
  newestBackup?: Date;
