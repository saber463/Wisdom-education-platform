 ...
 * 清理占用端口的进程
 */
export async function clearOccupiedPorts(): Promise<RepairResult> {
 ...
 * 清理临时文件和缓存
 */
export function cleanupTempFilesAndCache(): RepairResult {
  const details: string[] = [];
  const pathsToClean = [
 ...
 * 检查MySQL连接
 */
export function checkMySQLConnection(): RepairResult {
  const details: string[] = [];
 ...
 * 检查依赖完整性
 */
export function checkDependencies(): RepairResult {
  const details: string[] = [];
 ...
 * 检查并创建配置文件
 */
export function checkAndCreateConfigFiles(): RepairResult {
  const details: string[] = [];
  let created = 0;
 ...
 * 执行完整的应急修复
 */
export async function performEmergencyRepair(): Promise<BatchRepairResult> {
