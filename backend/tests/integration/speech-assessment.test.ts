...
describe('口语评测功能集成测试', () => {
  ...
  test('20.6 音频处理资源限制（并发≤5，CPU≤20%，内存≤150MB）', async () => {
    ...
    const resourceLimits: {
      ...
      cpu_percent_limit: number;
      memory_mb_limit: number;
      task_queue: Array<{ task_id: number; status: string }>;
    } = {
  ...
  try {
    ...
    const [basicMemberResult] = await connection.execute(
      `INSERT INTO users (username, password_hash, real_name, role, status)
       VALUES (?, ?, ?, ?, ?)`,
      ['test_basic_speech', 'hash123', '基础会员', 'student', 'active']
    );
    ...
    const [premiumMemberResult] = await connection.execute(
      `INSERT INTO users (username, password_hash, real_name, role, status)
       VALUES (?, ?, ?, ?, ?)`,
      ['test_premium_speech', 'hash123', '高级会员', 'student', 'active']
    );
