    ...
    <aside class="layout-sidebar" :class="{ collapsed: sidebarCollapsed }">
      <div class="layout-logo">
        ...
        <div v-if="!sidebarCollapsed" class="logo-text">
          ...
          <div class="logo-sub">PARENT · V2.0</div>
        </div>
      </div>
      ...
      <header class="layout-topbar">
        <div class="topbar-left">
          <el-breadcrumb separator="›">
            <el-breadcrumb-item :to="{ path: '/parent/dashboard' }">首页</el-breadcrumb-item>
...
import {
  HomeFilled, DataAnalysis, Warning, ChatDotRound,
...
const navItems = [
  { path: '/parent/dashboard',   label: '工作台',   icon: HomeFilled   },
  { path: '/parent/monitor',     label: '学情监控', icon: DataAnalysis },
  { path: '/parent/weak-points', label: '薄弱点',   icon: Warning      },
  { path: '/parent/messages',    label: '家校留言', icon: ChatDotRound },
  { path: '/parent/reports',     label: '成绩报告', icon: DataLine     },
]

const isActive = (path: string) => {
  if (path === '/parent/dashboard') return route.path === path
...
.layout-sidebar {
  width: 220px;
  background: #111111;
  border-right: 1px solid rgba(255, 183, 0, 0.08);
  display: flex;
...
.logo-icon {
  ...
  height: 36px;
  flex-shrink: 0;
  background: linear-gradient(135deg, #FFB700, #FF4B6E);
  border-radius: 10px;
  display: flex;
  align-items: center;
...
.user-avatar {
  border: 2px solid rgba(255,183,0,0.3) !important;
