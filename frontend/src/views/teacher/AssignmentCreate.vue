      ...
      <el-card class="form-card">
        ...
        >
          ...
          <el-form-item label="作业标题" prop="title">
            <el-input v-model="formData.title" placeholder="请输入作业标题" maxlength="200" show-word-limit />
          </el-form-item>
          
          <el-form-item label="作业描述" prop="description">
            <el-input v-model="formData.description" type="textarea" placeholder="请输入作业描述" :rows="3" />
          </el-form-item>
          ...
          <el-row :gutter="20">
            <el-col :span="12">
              <el-form-item label="截止时间" prop="deadline">
                <el-date-picker v-model="formData.deadline" type="datetime" placeholder="请选择截止时间" style="width: 100%" :disabled-date="disabledDate" />
              </el-form-item>
            </el-col>
            <el-col :span="12">
          ...
          <div class="questions-section">
            <div v-for="(question, index) in formData.questions" :key="index" class="question-item">
              <div class="question-header">
                ...
                <el-button type="danger" size="small" :icon="Delete" @click="removeQuestion(index)" :disabled="formData.questions.length <= 1">删除</el-button>
              </div>
              
              <el-row :gutter="20">
                <el-col :span="8">
                  <el-form-item :prop="`questions.${index}.questionType`" :rules="[{ required: true, message: '请选择题型', trigger: 'change' }]" label="题型">
                    <el-select v-model="question.questionType" placeholder="选择题型" style="width: 100%">
                ...
                <el-col :span="8">
                  <el-form-item :prop="`questions.${index}.score`" :rules="[{ required: true, message: '请输入分值', trigger: 'blur' }]" label="分值">
                    <el-input-number v-model="question.score" :min="1" :max="100" style="width: 100%" />
                  </el-form-item>
                </el-col>
                <el-col :span="8">
                  <el-form-item label="知识点">
                    <el-select v-model="question.knowledgePointId" placeholder="选择知识点" clearable style="width: 100%">
              ...
              <el-form-item :prop="`questions.${index}.questionContent`" :rules="[{ required: true, message: '请输入题目内容', trigger: 'blur' }]" label="题目内容">
                <el-input v-model="question.questionContent" type="textarea" placeholder="请输入题目内容" :rows="2" />
              </el-form-item>
              
              <el-form-item v-if="isObjectiveQuestion(question.questionType)" :prop="`questions.${index}.standardAnswer`" :rules="[{ required: true, message: '客观题必须提供标准答案', trigger: 'blur' }]" label="标准答案">
                <el-input v-model="question.standardAnswer" placeholder="请输入标准答案（客观题必填）" />
              </el-form-item>
              
              <el-form-item v-else label="参考答案">
                <el-input v-model="question.standardAnswer" type="textarea" placeholder="请输入参考答案（主观题可选）" :rows="2" />
              </el-form-item>
            </div>
            
            <el-button type="primary" plain @click="addQuestion" :icon="Plus">添加题目</el-button>
          </div>
          ...
          <div class="form-actions">
            ...
            <el-button type="primary" @click="handleSubmit" :loading="saving">创建并发布</el-button>
          </div>
        </el-form>
      </el-card>
    </div>
...
async function handleSaveDraft() {
  ...
  try {
    ...
    if (error !== false) { console.error('[创建作业] 保存草稿失败:', error); ElMessage.error(error.response?.data?.message || '保存失败') }
...
async function handleSubmit() {
  ...
  try {
    ...
    if (error !== false) { console.error('[创建作业] 创建发布失败:', error); ElMessage.error(error.response?.data?.message || '创建失败') }
  } finally { saving.value = false }
}
