...
async function testAdjustmentLogAPI() {
    ...
    try {
        ...
        const response1 = await axios.get(`${API_BASE_URL}/api/ai-learning-path/adjustment-log`, { headers });
        ...
        const response2 = await axios.get(`${API_BASE_URL}/api/ai-learning-path/adjustment-log?learning_path_id=1`, { headers });
        console.log(`✓ 成功获取路径1的 ${response2.data.data.total} 条调整日志`);
        console.log();
        // 测试3: 按调整类型筛选
        console.log('测试3: 按调整类型筛选 (adjustment_type=knowledge_evaluation)');
        const response3 = await axios.get(`${API_BASE_URL}/api/ai-learning-path/adjustment-log?adjustment_type=knowledge_evaluation`, { headers });
        console.log(`✓ 成功获取知识点评估类型的 ${response3.data.data.total} 条调整日志`);
        console.log();
        // 测试4: 自定义返回数量
        console.log('测试4: 自定义返回数量 (limit=5)');
        const response4 = await axios.get(`${API_BASE_URL}/api/ai-learning-path/adjustment-log?limit=5`, { headers });
        ...
        const response5 = await axios.get(`${API_BASE_URL}/api/ai-learning-path/adjustment-log?learning_path_id=1&adjustment_type=knowledge_evaluation&limit=10`, { headers });
 ...
 * - 显示详细的调整信息
 */
//# sourceMappingURL=test-adjustment-log-api.js.map