  ...
  try {
    ...
    const [student1Result] = await connection.execute(
      `INSERT INTO users (username, password_hash, real_name, role, status)
       VALUES (?, ?, ?, ?, ?)`,
      ['test_student_collab_1', 'hash123', '协作学习测试学生1', 'student', 'active']
    );
    student1Id = (student1Result as any).insertId;
    
    // 创建测试学生2
    const [student2Result] = await connection.execute(
      `INSERT INTO users (username, password_hash, real_name, role, status)
       VALUES (?, ?, ?, ?, ?)`,
      ['test_student_collab_2', 'hash123', '协作学习测试学生2', 'student', 'active']
    );
    student2Id = (student2Result as any).insertId;
    
    // 创建测试学生3
    const [student3Result] = await connection.execute(
      `INSERT INTO users (username, password_hash, real_name, role, status)
       VALUES (?, ?, ?, ?, ?)`,
      ['test_student_collab_3', 'hash123', '协作学习测试学生3', 'student', 'active']
    );
    student3Id = (student3Result as any).insertId;
